...
print (output.center(100, "*"))
print (output2.center(100, "*"))
tcs = [P[0], P[1], P[2], P[3], P[4], P[5]]
expected_results = [T[0][0],T[1][0],T[2][0],T[3][0],T[4][0],T[5][0], ]
...
for (ht_i,sp_i) in (tcs):
    ...
    if (expected_results[i] ==3 ):
        exp_class = "Negative"
    elif  (expected_results[i] ==8 ):
        exp_class = "Positive"
...
for (ht_i,sp_i) in (P):
    ...
    if ((p_f > 0.5) and (T[i][0] == 8)):
        correct_count = correct_count + 1
    elif ((p_f < 0.5) and (T[i][0] == 3)):
        correct_count = correct_count + 1
    else:
...
for (ht_i,sp_i) in (P):
    ...
    pd_f = get_pd (np.array([ht_i,sp_i]), MuF, CovF)
    Pf1 = bayesian(Nm, pd_m, Nf, pd_f)
    if ((Pf1 > 0.5) and (T[i][0] == 8)):
        correct_count = correct_count + 1
    elif ((Pf1 < 0.5) and (T[i][0] == 3)):
        correct_count = correct_count + 1
    else:
        incorrect_count = incorrect_count + 1
    i=i+1
...
wb = openpyxl.load_workbook('Assignment_3_ results_v2.xlsx')
...
#    
wb.save('Assignment_3_ results_v2.xlsx')
